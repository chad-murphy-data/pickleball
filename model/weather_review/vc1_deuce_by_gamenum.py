...
for mid,gs in gm.items():
    ...
    for g in gs:
        ...
        srows.append(dict(ev=ev,hour=int(hk[11:13]),day=hk[:10],wind=w[0],gust=w[1],temp=w[2],
                          ...
                          arm=lab.get(ev),nb=len(gs),tour=g["tour"]))
O=[r for r in srows if r["arm"]=="outdoor" and r["tour"]=="PPA"]
C=[("w",lambda r:r["wind"]/10.),("t",lambda r:r["temp"]/10.),
   ("g",lambda r:r["eta"]),("g2",lambda r:r["eta"]**2)]
FED=[lambda r: r["ev"]+"|"+r["day"], lambda r: r["hour"]]
def D(sub,tag,fes=FE,y=lambda r:r["deuce"],sc=100.,dec=2):
    if len(sub)<400: print("  %-46s n=%d too thin"%(tag,len(sub))); return
    rep(tag,fit(sub,C,fes,y),sc,"",dec)
print("\n=== PPA outdoor, deuce by GAME NUMBER (game 2 is NOT outcome-selected) ===")
for gn in (1,2,3):
    s=[r for r in O if r["gn"]==gn]
    print("   game %d  n=%d  deuce base %.4f" % (gn,len(s),np.mean([r["deuce"] for r in s]) if s else 0))
    D(s,"     event+hour FE")
    D([r for r in s if r["tru"]],"     TRUE game hour only")
print("\n=== same, MARGIN (pts) ===")
for gn in (1,2,3):
    D([r for r in O if r["gn"]==gn],"   game %d margin"%gn,y=lambda r:float(r["margin"]),sc=1.,dec=3)
print("\n=== games 1+2 pooled (both unconditional), PPA outdoor ===")
D([r for r in O if r["gn"]<=2],"   g1+g2 deuce")
D([r for r in O if r["gn"]<=2],"   g1+g2 deuce, event-DAY FE",FED)
D([r for r in O if r["gn"]<=2 and r["tru"]],"   g1+g2 deuce, TRUE hour")
