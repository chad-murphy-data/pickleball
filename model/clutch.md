...
> **THIS INDEX DOES NOT SURVIVE A SIMULATED NULL (2026-08-03).** See
> `model/clutch_endogeneity.py`. Simulating the archive with ZERO clutch for
> anyone — real games, real players, real skill, constant per-side rally
> probability, real side-out dynamics — and running THIS estimator on it
> yields sd(clutch) 0.0301 against the real 0.0301, a ratio of 1.00x, and a
> skill correlation of +0.522 against the real +0.59. A world with no clutch
> reproduces this file's numbers almost exactly.
>
> The cause: which rallies count as "big" is computed from the score, and the
> score is produced by the rallies, so the label is endogenous to the outcome.
> §2's permutation null does NOT control for this — shuffling levz within a
> game destroys the leverage-outcome association entirely, so it tests against
> "no association" when the artefact IS an association generated by the score
> dynamics.
>
> **What this retracts:** §2 ("the trait exists"), §3's reliability as evidence
> of a trait (a stable artefact replicates too), and §4's per-player residual
> reads. The `content/clutch/` posts are built on these and need revisiting.
> §5 is unaffected and was always the robust part: clutch predicts nothing.
>
> The two-regime rebuild (`clutch_regimes.md`, `data/clutch_ratings.csv`) fails
> the same test just as badly and was NOT adopted.
