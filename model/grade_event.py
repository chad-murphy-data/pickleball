...
DUPR. Writes data/accuracy.json (machine) and model/accuracy.md (the
just-open-it scorecard) so the numbers live in the repo instead of being
...
Honesty notes baked into the output:
  * Model values come from the most recent v2 fit (data/v2_players.csv).
    This weekend's matches were NOT in that fit, so they are genuinely
    out-of-sample.
  * DUPR = latest synced rating per player (data/platform_ratings.csv);
    a Challenger weekend barely moves a pro's number, but it is "latest",
    not strictly pre-match, so treat rank ties gently.
  * DUPR gets winner accuracy only: it is a rating, not a probability, so
    there is no honest Brier to quote for it.
  * Sources are this session's raw/ + tournament_state.json, so the grade
    covers the currently-completed slate; re-run to refresh as more
    matches finish.
"""
...
def dupr_pick(a, b, dupr):
    """+1 if DUPR favors pair a, -1 pair b, 0 tie/unknown."""
...
# ---------------------------------------------------------------- MLP
def grade_mlp(event_title, on_date, vals, dupr):
    """Game- and matchup-level grade of one MLP event-day from raw detail.

    Sourced from the raw tl_matchups_short feed for the date (the
    tournament_state 'completed' summary can undercount a day's slate)."""
    uuids = []
    for fp in glob.glob(str(ROOT / "raw" / "tl_matchups_short" / "*" /
                            f"{on_date}.json")):
        for m in (json.loads(Path(fp).read_text()).get("data") or []):
            if m.get("uuid"):
                uuids.append(m["uuid"])
    g_pk = [0, 0]
    g_du = [0, 0]
    ...
    for u in uuids:
        fp = ROOT / "raw" / "matchup_data" / f"{u}.json"
        if not fp.exists():
            continue
        d = json.loads(fp.read_text())["data"]
        a1, a2 = d["teamOneAbbreviation"], d["teamTwoAbbreviation"]
        s1, s2 = d["teamOneScore"], d["teamTwoScore"]
        win_a = s1 > s2
        game_ps = []
        du_favs = []
        for g in d["matches"]:
    ...
    return {
        "event": event_title, "date": on_date, "level": "matchup+game",
...
# ---------------------------------------------------------------- PPA
def grade_ppa(tournament_sub, vals, dupr):
    st = json.loads((DATA / "tournament_state.json").read_text())
    divs_out = []
    for ev in st.get("ppa", []):
        if tournament_sub.lower() not in ev["tournament"].lower():
            continue
        ...
        return {"event": ev["tournament"], "level": "match", "divisions": divs_out}
    return None
...
def render_md(mlp, ppa):
    L = ["# Accuracy scorecard — PICKLES vs DUPR",
         "",
         "_Auto-generated by `python model/grade_event.py`. Winner accuracy on "
         "completed pro doubles; model values are from the last v2 fit "
         "(out-of-sample for these matches), DUPR is each player's latest "
         "synced rating. DUPR shows winner accuracy only — it's a rating, not "
         "a probability. Uses each match's **actual** lineups (a reproducible "
         "retrodiction) — distinct from the frozen pre-match receipts in "
         "`receipts.json`. Brier is game/match level._", ""]
    if mlp:
        m = mlp["matchups"]
        g = mlp["games"]
        L += [f"## {mlp['event']} — {mlp['date']} (MLP, team format)", "",
              ...
              f"{mlp['brier']} · "
              f"DUPR {g['dupr']} ({pct(g['dupr_correct'], g['dupr_n'])})**", "",
              "| Actual | PICKLES | ✓ | DUPR | ✓ |",
              "|---|---|:--:|---|:--:|"]
        for r in mlp["rows"]:
...
def main():
    vals = load_values()
    dupr = load_dupr()
    mlp = grade_mlp("MLP San Diego", "2026-07-18", vals, dupr)
    ppa = grade_ppa("Macon", vals, dupr)
    out = {"generated_from": "raw/ + data/tournament_state.json",
           "calibration": {"a": CAL["a"], "b": CAL["b"], "eps": EPS},
           "mlp": mlp, "ppa": ppa}
    (DATA / "accuracy.json").write_text(json.dumps(out, indent=1))
    (ROOT / "model" / "accuracy.md").write_text(render_md(mlp, ppa))
