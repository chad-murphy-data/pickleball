...
SCHEMA_VERSION = "ball-label-v2.1"
...
let displayedTime=null,displayedFrame=null,frameReady=false,seekAttempts=0,proposalWasVisible=false;
...
function render(){const s=sample(),a=labels[idx];const done=Object.keys(labels).length;const drift=displayedTime==null?'':` · shown frame ${displayedFrame} · drift ${((displayedTime-s.nominal_t_s)*CFG.video.fps).toFixed(2)}f`;const ready=frameReady?'':` · <span class=\"warn\">${displayedFrame==null?'seeking…':`decoded ${displayedFrame}, retrying…`}</span>`;status.innerHTML=`sample <b>${idx+1}</b>/${CFG.samples.length} · source frame ${s.source_frame} · ${s.nominal_t_s.toFixed(4)}s${drift}${ready} · label <b>${a?a.visibility:'—'}</b>`;reason.textContent=s.reason+(s.blind?' · blind':'');modeEl.textContent=mode==='V'?'':`${mode} armed`;fill.style.width=`${100*done/CFG.samples.length}%`;proposalWasVisible=proposalAllowed(s)&&!a;proposalState.textContent=Object.keys(proposals).length?`${Object.keys(proposals).length} proposals loaded${s.blind&&!a?' · current frame blinded':''}`:'';draw()}
...
new ResizeObserver(draw).observe(video);video.onseeked=()=>{if(!video.requestVideoFrameCallback){displayedTime=video.currentTime;displayedFrame=decodedFrame(displayedTime);frameReady=displayedFrame===sample().source_frame}render()};
function record(visibility,x=null,y=null,method='click'){if(!frameReady)return;labels[idx]={visibility,x,y,displayed_t_s:displayedTime,displayed_frame:displayedFrame,input_method:method,proposal_visible:proposalWasVisible};mode='V';save();go(idx+1)}
...
importButton.onclick=()=>labelPick.click();labelPick.onchange=()=>{const rd=new FileReader();rd.onload=()=>{for(const r of parseCSV(rd.result)){const i=CFG.samples.findIndex(s=>s.source_frame===+r.source_frame);if(i<0)continue;labels[i]={visibility:r.visibility,x:r.x===''?null:+r.x,y:r.y===''?null:+r.y,displayed_t_s:r.displayed_t_s===''?null:+r.displayed_t_s,displayed_frame:r.displayed_frame===''?null:+r.displayed_frame,input_method:r.input_method||'import',proposal_visible:r.proposal_visible==='1'}}save();render()};rd.readAsText(labelPick.files[0])};
exportButton.onclick=()=>{const cols=['schema','video_name','video_duration_s','fps','fps_rational','rally','sample_index','source_frame','nominal_t_s','displayed_t_s','displayed_frame','seek_drift_frames','x','y','visibility','sampling_reason','blind','input_method','proposal_visible'];let out=cols.join(',')+'\n';for(let i=0;i<CFG.samples.length;i++){const a=labels[i];if(!a)continue;const s=CFG.samples[i],drift=a.displayed_t_s==null?'':(a.displayed_t_s-s.nominal_t_s)*CFG.video.fps;const row=[CFG.schema,CFG.video.name,CFG.video.duration_s,CFG.video.fps,CFG.video.fps_rational,CFG.rally,i,s.source_frame,s.nominal_t_s,a.displayed_t_s,a.displayed_frame,drift,a.x,a.y,a.visibility,s.reason,s.blind?1:0,a.input_method,a.proposal_visible?1:0];out+=row.map(csvEscape).join(',')+'\n'}const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([out],{type:'text/csv'}));a.download=`ball_labels_v2_r${CFG.rally}.csv`;a.click()};
render();
</script></body></html>'''
