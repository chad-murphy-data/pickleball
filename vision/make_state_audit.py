...
done · <kbd>X</kbd> no-contact episode (fake / never struck —
close it with <kbd>E</kbd> too, so negatives get durations) · <kbd>R</kbd> service routine starts ·
...
nearest mark. Mark keys act on the episode <b>nearest the playhead</b>,
so you can seek back into any old episode to close, flag, or fix it. Gray ticks = your old taps (red = fast): navigate by
...
document.addEventListener("keydown", e => {
  ...
  // I/E/X/F/H/P act on the episode NEAREST the playhead (within 3s),
  // not just the latest — so old episodes can be revisited: seek into
  // one and press the key (E closes it, X flags it, I re-marks impact).
  const nearestHit = () => {
    let best = null, bd = 3.0;
    p.hits.forEach(h => {
      if (h.s == null) return;
      const d = (t >= h.s && (h.e == null || t <= h.e)) ? 0
        : Math.min(Math.abs(t - h.s),
                   h.e != null ? Math.abs(t - h.e) : 1e9);
      if (d < bd) { bd = d; best = h; }
    });
    return best;
  };
  ...
  else if (k === "i") { const h = nearestHit();
    if (h) h.i = +t.toFixed(3); }
  else if (k === "e") { const h = nearestHit();
    if (h) h.e = +t.toFixed(3); }
  else if (k === "x") { const h = nearestHit();
    if (h) h.nc = !h.nc; }
  else if (k === "f") { const h = nearestHit();
    if (h) h.hand = h.hand === "fh" ? null : "fh"; }
  else if (k === "h") { const h = nearestHit();
    if (h) h.hand = h.hand === "bh" ? null : "bh"; }
  else if (k === "p") { const h = nearestHit();
    if (h) h.poach = !h.poach; }
